...
const defaultData: OrganizationsData = {
  type: "organizations",
  name: "",
  role: "",
  startDate: "",
...
const OrganizationsSectionModal = ({ section, saving, onCancel, onSave }: Props) => {
  const [data, setData] = useState<OrganizationsData>(defaultData);

  useEffect(() => {
    try {
      const parsed = JSON.parse(section.description || "");
      if (parsed?.type === "organizations") {
        setData({
          type: "organizations",
          name: parsed.name ?? "",
          role: parsed.role ?? "",
  ...
  const handleChange =
    (field: keyof OrganizationsData) => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
  ...
  return (
    <div className="w-full max-w-4xl rounded-2xl bg-white p-8 shadow-2xl">
      <h1 className="mb-8 text-center text-3xl font-bold">Thêm tổ chức</h1>
      <div className="mb-8 space-y-6">
        <div>
          <label className="mb-2 block text-gray-700">Tên tổ chức</label>
          <input
            type="text"
            value={data.name}
            onChange={handleChange("name")}
            placeholder="Câu lạc bộ, tổ chức..."
...
export default OrganizationsSectionModal;
