...
const ExperienceSectionModal = ({ section, saving, onCancel, onSave }: Props) => {
  const [data, setData] = useState<ExperienceData>(defaultData);
  ...
  const handleChange = (field: keyof ExperienceData) => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
  ...
  return (
    <div className="w-full max-w-4xl rounded-2xl bg-white p-8 shadow-2xl">
      <h1 className="mb-8 text-center text-3xl font-bold">Kinh nghiệm làm việc</h1>

      <div className="mb-8 space-y-6">
        <div>
          <label className="mb-2 block text-gray-700">Vị trí</label>
          <input
            type="text"
            value={data.position}
        ...
        <div>
          <label className="mb-2 block text-gray-700">Tên đơn vị công tác</label>
          <input
            type="text"
            value={data.company}
        ...
        <div>
          <label className="mb-2 block text-gray-700">Thời gian làm việc</label>
        ...
        <div>
          <label className="mb-2 block text-gray-700">Mô tả chi tiết công việc</label>
          <textarea
